...
if not HAND_LANDMARKS_CSV.exists():
    raise FileNotFoundError(f"Hand landmarks CSV file not found: {HAND_LANDMARKS_CSV}")
...
class LandmarksDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    ...
    def __getitem__(self, idx: int) -> tuple[np.ndarray, int]:
        ...
        # Convert to torch tensors so DataLoader yields tensors compatible
        # with the training loops which expect `torch.Tensor` inputs.
