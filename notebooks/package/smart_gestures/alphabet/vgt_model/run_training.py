...
def main(args):
    ...
    parser.add_argument("--scheduler_factor", type=float, default=0.5, help="ReduceLROnPlateau factor or ignored")
    parser.add_argument("--scheduler_patience", type=int, default=3, help="ReduceLROnPlateau patience or ignored")
    parser.add_argument("--step_size", type=int, default=10, help="StepLR step_size if scheduler=step")
    parser.add_argument("--gamma", type=float, default=0.1, help="StepLR decay factor if scheduler=step")
    parser.add_argument("--early_stop_patience", type=int, default=5)
    parser.add_argument("--early_stop_min_delta", type=float, default=0.0)
    parser.add_argument("--monitor", type=str, default="val_loss", choices=["val_loss", "val_acc"], help="Metric for early stop/checkpoint mode")
    ...
    if parsed_args.scheduler == "plateau":
        scheduler_kwargs = {"factor": parsed_args.scheduler_factor, "patience": parsed_args.scheduler_patience}
    elif parsed_args.scheduler == "step":
        scheduler_kwargs = {"step_size": parsed_args.step_size, "gamma": parsed_args.gamma}
